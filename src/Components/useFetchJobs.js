...
case 'get-data':
    return {...state, jobs:action.payload, loading:false, error: false };
case 'errors':
...
export default function useFetchJobs(page, params){
    const [state,dispatch] = useReducer(reducer, {loading: true, jobs:[], error: false, hasNextPage: false});
    ...
    let nextPageUrl = `${ProductionBaseUrl}?description=${params.description}&location=${params.location}&full_time=${params.full_time}&page=${page + 1}&markdown=true`;
        ...
        method: 'get'
            ...
            if(!res.ok){
                dispatch({type:'errors', payload:true});
       ...
       fetch(nextPageUrl, {
        ...
        method: 'get'
            ...
            dispatch({type:'has-next-page', payload:data.length !== 0});
        }).catch(error => {
        dispatch({type:'errors', payload:true});
        });
    }, [params, page]);
